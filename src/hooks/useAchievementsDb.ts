...
import { updateUserAchievements } from '@/server/db-api';
...
export const useAchievementsDb = () => {
  ...
  const syncWithServer = async (userData: any) => {
    try {
      // Use our API function to update user achievements
      const data = await updateUserAchievements(
        userData.userId,
        {
          completed: Array.isArray(userData.achievements) 
            ? userData.achievements.filter((a: Achievement) => a.completed) 
            : [],
